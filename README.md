...
╚══════════════════════════════════════════════════════════════════════════════╝
                    ...
                    │                    │                    │
         ┌──────────▼──────────┐  ┌─────▼──────┐  ┌─────────▼─────────┐
         │   gguf-tools/       │  │  GGUF File │  │     imatrix/       │
         │  deepseek4-quantize │  │  (81 GiB)  │  │  calibration data  │
         │  quants.c (IQ2/Q2)  │  └─────┬──────┘  └───────────────────┘
         └─────────────────────┘        │
                                        │ mmap()
╔═══════════════════════════════════════╪══════════════════════════════════════╗
...
║                                                                            ║
║  ┌──────────┐  ┌───────────┐  ┌──────────────┐  ┌────────────────────┐    ║
║  │  GGUF     │  │ Tokenizer │  │ Chat Template│  │  Session Manager   │    ║
║  │  Loader   │  │ (BPE)     │  │ (DSML tools) │  │  KV / Sync / Rebuild│   ║
║  │ Tensor    │  │ 129280    │  │ system/user/ │  │  Disk serialize    │    ║
║  │ Mapping   │  │ vocab     │  │ assistant/   │  │  Session save/load │    ║
║  └─────┬─────┘  │           │  │ tool roles   │  └─────────┬──────────┘    ║
║        │        └───────────┘  └──────────────┘            │               ║
║        │                                                    │               ║
║  ┌─────▼────────────────────────────────────────────────────▼──────────┐   ║
║  │                      GPU Graph Scheduler                             │   ║
║  │  ┌──────────────┐  ┌────────────┐  ┌─────────────┐  ┌────────────┐  │   ║
║  │  │ Prefill Graph│  │Decode Graph│  │ Layer Split │  │Power Mgmt  │  │   ║
║  │  │ (chunked)    │  │(autoregr.) │  │ (distrib.)  │  │(duty cycle)│  │   ║
║  │  └──────────────┘  └────────────┘  └─────────────┘  └────────────┘  │   ║
║  └─────────────────────────────────────────────────────────────────────┘   ║
║                                                                            ║
║                              ┌───▼────┐                                    ║
║                              │Sampler │  temp / top-p / top-k / min-p      ║
║                              └───┬────┘                                    ║
╚══════════════════════════════════╪══════════════════════════════════════════╝
                                   │
                    ┌──────────────┼──────────────┐
                    │              │              │
          ┌────────▼───────┐ ┌────▼───────┐ ┌────▼──────────┐
          │  ds4_gpu.h     │ │ CPU Path   │ │ ds4_iq2_tables│
          │  (~100 funcs)  │ │ (ref only) │ │ _cuda.inc     │
          └────────┬───────┘ └────────────┘ └───────────────┘
                   │
     ┌─────────────┼─────────────────┐
     │             │                 │
╔════▼════╗  ╔═════▼══════╗  ╔══════▼══════════╗
║  METAL  ║  ║   CUDA     ║  ║      ROCm       ║
║ macOS   ║  ║ DGX Spark  ║  ║  Strix Halo     ║
║ primary ║  ║ sm_121     ║  ║  gfx1151        ║
║         ║  ║            ║  ║                 ║
║ ds4_    ║  ║ ds4_cuda.cu║  ║ ds4_rocm.cu     ║
║ metal.m ║  ║            ║  ║ rocm/*.cuh (22) ║
║ metal/  ║  ╚════════════╝  ╚═════════════════╝
║ *.metal ║
║  (20)   ║
...
GPU Kernel Categories (metal/ + rocm/):
┌──────────┬──────────────────────────────────────────────────────┐
│ Category │ Kernels                                               │
├──────────┼──────────────────────────────────────────────────────┤
│ Dense    │ dense.metal     — matmul, Q4/Q8/IQ2 dequant+dot      │
│ MoE      │ moe.metal       — 256-expert Top-K routing, gate/up/ │
│          │                   down projection, expert aggregation │
│ Attn     │ flash_attn.metal— Flash Attention with MLA (Multi-   │
│          │                   head Latent Attention) compression  │
│ HC       │ dsv4_hc.metal   — Hyper-Connection residual gating   │
│ KV       │ dsv4_kv.metal   — KV cache read/write, raw↔latent    │
│ RoPE     │ dsv4_rope.metal — Rotary Position Encoding            │
│ Norm     │ norm.metal      — RMS Normalization                   │
│ Act.     │ glu.metal       — Gated Linear Unit                   │
│          │ softmax.metal   — Softmax                             │
│ Util     │ argsort, bin, cpy, concat, get_rows, set_rows,       │
│          │ sum_rows, repeat, unary                               │
│ Misc     │ dsv4_misc.metal — DeepSeek V4 specific misc ops       │
└──────────┴──────────────────────────────────────────────────────┘
...
║                           │                                                  ║
║  ┌─────────────────────┐  │  ┌──────────────────────────────────────────┐   ║
║  │ ds4 (CLI)            │  │  │ ds4-server (HTTP API)                    │   ║
║  │ ds4_cli.c            │  │  │ ds4_server.c + ds4_kvstore.c + rax.c    │   ║
║  │  · Interactive REPL  │  │  │                                          │   ║
║  │  · One-shot -p       │  │  │  GET  /v1/models                        │   ║
║  │  · /think /nothink   │  │  │  POST /v1/chat/completions (SSE stream) │   ║
║  │  · /ctx /power /read │  │  │  POST /v1/responses   (Codex CLI)       │   ║
║  │  · --dump-logprobs   │  │  │  POST /v1/messages    (Claude Code)     │   ║
║  │  · --dump-logits     │  │  │  POST /v1/completions                   │   ║
║  │  · --inspect model   │  │  │  · CORS, trace, host/port config        │   ║
║  └──────────┬───────────┘  │  │  · Disk KV cache persistence            │   ║
║             │              │  └──────────────────────────────────────────┘   ║
║             │              │                                                  ║
║  ┌──────────▼───────────┐  │  ┌──────────────────────────────────────────┐   ║
║  │ ds4-agent (Coding)   │  │  │ ds4-bench (Speed Test)                   │   ║
║  │ ds4_agent.c + web    │  │  │ ds4_bench.c                              │   ║
║  │  · File r/w/search   │  │  │  · Prefill throughput @ varying ctx      │   ║
║  │  · Shell execution   │  │  │  · Generation throughput                 │   ║
║  │  · Web search/fetch  │  │  │  · CSV export, SVG chart via plot_speed  │   ║
║  │  · Session save/load │  │  └──────────────────────────────────────────┘   ║
║  │  · /save /list       │  │                                                  ║
║  │  · /switch /del      │  │  ┌──────────────────────────────────────────┐   ║
║  └──────────────────────┘  │  │ ds4-eval (Quality Test)                  │   ║
║                            │  │ ds4_eval.c                               │   ║
║                            │  │  · GPQA, SuperGPQA, AIME, COMPSEC       │   ║
║                            │  │  · Official logit vector validation      │   ║
...
║                          │                                                   ║
║  ┌─────────────────────┐ │  ┌──────────────────────────────────────────┐    ║
║  │ KV Store             │ │  │ SSD Streaming                            │    ║
║  │ ds4_kvstore.c/.h     │ │  │ ds4_ssd.c/.h                             │    ║
║  │                      │ │  │                                          │    ║
║  │ · SHA1(content)→file │ │  │  · Expert hotlist preload               │    ║
║  │ · LRU eviction       │ │  │    (ds4_streaming_hotlist.inc,           │    ║
║  │ · Cross-quant accept │ │  │     13K lines, profiled routing stats)   │    ║
║  │ · Continuation/Cold  │ │  │  · mlock() expert cache locking          │    ║
║  │   save policies      │ │  │  · Auto cache budget (80% rule)          │    ║
║  │ · Tool-id exact-     │ │  │  · mmap + pread() zero-copy expert load  │    ║
║  │   replay maps        │ │  │  · mlock guard: refuse if can't lock     │    ║
║  │ · Trim & align       │ │  │  · Simulate-used-memory diagnostics      │    ║
║  └─────────────────────┘ │  └──────────────────────────────────────────┘    ║
║                          │                                                   ║
║  ┌─────────────────────┐ │  ┌──────────────────────────────────────────┐    ║
║  │ Distributed Inference│ │  │ Web Utilities (for ds4-agent)            │    ║
║  │ ds4_distributed.c/.h │ │  │ ds4_web.c/.h                             │    ║
║  │                      │ │  │                                          │    ║
║  │ · Coordinator/Worker │ │  │  · Google search (via web search API)    │    ║
║  │   roles              │ │  │  · Web page fetch & render               │    ║
║  │ · Layer-range split  │ │  └──────────────────────────────────────────┘    ║
║  │ · Pipeline prefill   │ │                                                   ║
║  │   (chunk N+1 while   │ │  ┌──────────────────────────────────────────┐    ║
║  │    worker on chunk N)│ │  │ Third-Party (in-tree)                    │    ║
║  │ · Worker-to-worker   │ │  │                                          │    ║
║  │   direct comm (no    │ │  │  · linenoise — REPL line editing         │    ║
║  │   coordinator relay) │ │  │  · rax       — Radix tree for server     │    ║
║  │ · KV state pull/push │ │  │               tool-ID prefix matching    │    ║
║  │   for session save   │ │  └──────────────────────────────────────────┘    ║
║  │ · Rolling hash KV    │ │                                                   ║
║  │   consistency check  │ │                                                   ║
║  └─────────────────────┘ │                                                   ║
...
║                                                                              ║
║  ┌───────────────────┐  ┌─────────────────────────┐  ┌───────────────────┐  ║
║  │  deploy_ds4.sh     │  │  ds4_bench_cli.py        │  │  ds4_bench_api.py │  ║
║  │                    │  │  (推荐，直调 ./ds4)       │  │  (HTTP streaming) │  ║
║  │  start/stop/       │  │                          │  │                   │  ║
║  │  restart/status/   │  │  ShareGPT → ./ds4 → CSV  │  │  ShareGPT → API   │  ║
║  │  logs/build/test/  │  │  测量: TTFT/prefill/gen  │  │  → CSV            │  ║
║  │  bench/cli/        │  └─────────────────────────┘  └───────────────────┘  ║
║  │  client/help       │                                                      ║
║  └───────────────────┘                                                       ║
║                                                                              ║
║  ┌───────────────────┐  ┌───────────────────┐  ┌─────────────────────────┐  ║
║  │ download_model.sh  │  │ gguf-tools/        │  │ dir-steering/           │  ║
║  │                    │  │                    │  │                         │  ║
║  │ · curl -C (resume) │  │ · quantize (IQ2/Q2 │  │ · build_direction.py    │  ║
║  │ · hf CLI (PRO)     │  │   /Q4/Q8)         │  │ · run_sweep.py          │  ║
║  │ · symlink auto     │  │ · imatrix collect  │  │ · steering vectors      │  ║
║  │ · token auth       │  │ · quality scoring  │  │ · verbosity control     │  ║
║  └───────────────────┘  │ · mixed expert     │  └─────────────────────────┘  ║
║                         │   splicing         │                               ║
║  ┌───────────────────┐  └───────────────────┘                               ║
║  │ speed-bench/       │                                                      ║
...
║  │ · plot_speed.py    │  │ tests/             │                               ║
║  │ · *.csv bench data │  │                    │                               ║
║  │ · *.svg charts     │  │ · ds4_test.c       │                               ║
║  │ · promessi_sposi   │  │ · test-vectors/    │                               ║
║  │   (bench prompt)   │  │   official.vec     │                               ║
║  └───────────────────┘  │   local-golden.vec  │                               ║
║                         │ · long context tests│                               ║
║                         │ · test_q4k_dot      │                               ║
║                         │ · cuda_long_context │                               ║
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                    INFERENCE DATA FLOW (single request)                      ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  User Prompt                                                                 ║
║      │                                                                       ║
║      ▼                                                                       ║
║  ┌─────────┐    ┌─────────────┐    ┌────────────────────────────────────┐   ║
║  │Tokenizer│───▶│Chat Template│───▶│        GPU Graph Execution          │   ║
║  │ BPE     │    │ DSML tools  │    │                                    │   ║
║  │ 129K    │    │ system/user │    │  For each of 27 MoE layers:        │   ║
║  │ vocab   │    │ /assistant  │    │   1. Hyper-Connection gate          │   ║
║  └─────────┘    └─────────────┘    │   2. Router: top-8 of 256 experts   │   ║
║                                    │   3. Shared Expert (always active)  │   ║
║  ┌──────────────────────────────┐  │   4. Routed MoE (8 experts)         │   ║
║  │     KV Cache (MLA)           │  │   5. Flash Attention (latent KV)    │   ║
║  │                              │  │   6. RMS Norm + Residual            │   ║
║  │  Raw Buffer (4352 slots)     │◀─│                                      │   ║
║  │  Compressed Latent (16K+)    │  │   + 1 Shared Expert layer            │   ║
║  │  Disk: ~/.ds4/server-kv/     │  │   + Compressor/Indexer (latent KV)   │   ║
║  └──────────────────────────────┘  └────────────────┬───────────────────┘   ║
║                                                     │                       ║
║  ┌──────────────────────────────┐                    ▼                       ║
║  │      SSD Streaming           │  ┌────────────────────────────────────┐   ║
║  │  (optional, if model > RAM)  │  │           Sampler                  │   ║
║  │                              │  │  temperature / top_p / top_k /     │   ║
║  │  Cache Miss → mmap + pread() │  │  min_p / seed                     │   ║
║  │  Hot Expert → mlock in RAM   │  └────────────────┬───────────────────┘   ║
║  └──────────────────────────────┘                    │                       ║
║                                                     ▼                       ║
║                                               Next Token → loop              ║
║                                                                              ║
...
║                                                                              ║
║  ┌─────────────┐   ┌──────────────┐   ┌─────────────┐   ┌───────────────┐   ║
║  │ Claude Code  │   │   OpenCode   │   │  Codex CLI  │   │  curl / any   │   ║
║  │              │   │              │   │             │   │  HTTP client  │   ║
║  │ Anthropic    │   │ OpenAI       │   │ OpenAI      │   │               │   ║
║  │ Messages API │   │ Compatible   │   │ Responses   │   │ Chat / Models │   ║
║  └──────┬───────┘   └──────┬───────┘   └──────┬──────┘   └───────┬───────┘   ║
║         │                  │                  │                  │           ║
║         └──────────────────┴──────────────────┴──────────────────┘           ║
║                                    │                                         ║
...
```text
ds4/
├── Makefile                      # Build system (Metal/CUDA/ROCm/CPU)
├── deploy_ds4.sh                 # 一键部署脚本
├── download_model.sh             # 模型下载 (HF)
│
├── Core Engine
│   ├── ds4.h                     # Public API header
│   ├── ds4.c                     # Core: loader, tokenizer, CPU path, sessions, sampling
│   ├── ds4_gpu.h                 # GPU abstraction layer (~100 function decls)
│   ├── ds4_streaming_hotlist.inc # Pre-profiled MoE expert hotlist (13K lines)
│   └── ds4_iq2_tables_cuda.inc   # IQ2 lookup tables (CUDA)
│
├── GPU Backends
│   ├── ds4_cuda.cu               # CUDA backend (DGX Spark / NVIDIA)
│   ├── ds4_metal.m               # Metal backend (macOS)
│   ├── ds4_rocm.cu + ds4_rocm.h  # ROCm backend (Strix Halo)
│   ├── metal/                    # 20 Metal shader kernels
│   │   ├── dense.metal           #   Matmul + dequant
│   │   ├── moe.metal             #   Mixture-of-Experts (largest, 181 KB)
│   │   ├── flash_attn.metal      #   Flash Attention (MLA)
│   │   ├── dsv4_hc.metal         #   Hyper-Connection
│   │   ├── dsv4_kv.metal         #   KV cache ops
│   │   ├── dsv4_rope.metal       #   Rotary Position Encoding
│   │   ├── dsv4_misc.metal       #   Misc DSv4-specific ops
│   │   ├── norm.metal            #   RMS Normalization
│   │   ├── softmax.metal         #   Softmax
│   │   ├── glu.metal             #   Gated Linear Unit
│   │   └── argsort/bin/cpy/concat/get_rows/set_rows/
│   │       sum_rows/repeat/unary.metal
│   └── rocm/                     # 22 HIP kernel headers
│       ├── ds4_rocm_attention.cuh
│       ├── ds4_rocm_moe.cuh + ds4_rocm_moe_launch.cuh
│       ├── ds4_rocm_matmul.cuh / compressor / indexer / router
│       ├── ds4_rocm_shared_expert.cuh
│       ├── ds4_rocm_fp8_kv.cuh   # FP8 KV cache
│       ├── ds4_rocm_hc.cuh       # Hyper-Connection
│       ├── ds4_rocm_hipblaslt.cuh# hipBLASLt integration
│       └── ...
│
├── Applications
│   ├── ds4_cli.c                 # CLI REPL + one-shot
│   ├── ds4_server.c              # HTTP API server (OpenAI + Anthropic)
│   ├── ds4_agent.c               # Native coding agent TUI
│   ├── ds4_bench.c               # Speed benchmark
│   ├── ds4_eval.c                # Capability evaluation
│   ├── ds4_help.c + ds4_help.h   # Unified --help text
│   └── ds4_web.c + ds4_web.h     # Web fetch/search (for agent)
│
├── Supporting Infrastructure
│   ├── ds4_kvstore.c + .h        # Disk KV cache persistence (LRU, SHA1)
│   ├── ds4_ssd.c + .h            # SSD streaming (mlock cache, auto budget)
│   └── ds4_distributed.c + .h    # Multi-machine layer pipeline
│
├── Third-Party (in-tree)
│   ├── linenoise.c + .h          # REPL line editing (by antirez)
│   ├── rax.c + .h + rax_malloc.h # Radix tree for server tool-ID lookup
│   └── gguf-tools/               # GGUF quantizer + imatrix tools
│       ├── deepseek4-quantize.c
│       ├── quants.c + quants.h
│       ├── imatrix/              #   Calibration data collection
│       ├── quality-testing/      #   Score vs official continuations
│       └── mixed/                #   Mixed expert layer splicing
│
├── Scripts & Tooling
│   ├── ds4_bench_cli.py          # ShareGPT CLI benchmark (recommended)
│   ├── ds4_bench_api.py          # ShareGPT API benchmark
│   ├── speed-bench/
│   │   ├── plot_speed.py         #   SVG chart generator
│   │   └── promessi_sposi.txt    #   Standard long benchmark prompt
│   └── dir-steering/
│       └── tools/
│           ├── build_direction.py#   Steering vector builder
│           └── run_sweep.py      #   Scale sweep on prompt set
│
├── Tests
│   ├── ds4_test.c                # Main test runner (server, vectors, kernel)
│   ├── test-vectors/             # Official + local-golden logit fixtures
│   ├── long_context_*.txt        # Long context test prompts
│   ├── test_q4k_dot.c            # Q4K dot product unit test
│   └── cuda_long_context_smoke.c # CUDA long context smoke test
│
├── Documentation
│   ├── README.md                 # Project overview (this file)
│   ├── USER_MANUAL.md            # Chinese user manual
│   ├── COMPARISON.md             # DS4 vs llama.cpp vs vLLM
│   ├── MODEL_CARD.md             # DeepSeek V4 model card
│   ├── AGENT.md                  # Agent developer notes
│   ├── CONTRIBUTING.md           # Contribution guide
│   ├── QA_BEFORE_RELEASES.md     # Release QA checklist
│   ├── STRIXHALO.md              # ROCm setup guide
│   └── misc/
│       ├── RESPONSE_API.md       # Responses API design notes
│       ├── ANTHROPIC_LIVE_CONTINUATION.md
│       └── COMPACT.md            # Agent context compaction design
│
└── Build Outputs (make targets)
    ├── ds4        (CLI chat)
    ├── ds4-server (HTTP API)
    ├── ds4-agent  (Coding agent)
    ├── ds4-bench  (Speed test)
    └── ds4-eval   (Quality test)
```

### Quick Start with `deploy_ds4.sh`
...
# 常用命令
./deploy_ds4.sh status          # 查看运行状态、系统内存、GPU 温度
./deploy_ds4.sh logs            # 实时日志
./deploy_ds4.sh stop            # 停止服务
./deploy_ds4.sh restart         # 重启

# Benchmark
./deploy_ds4.sh bench cli 50 256    # CLI 模式，50 条，256 max_tokens
./deploy_ds4.sh bench api 50 256    # API 模式（需 server 运行中）

# 客户端配置（Claude Code / OpenCode / Codex）
./deploy_ds4.sh client

# 自定义参数
DS4_CTX=100000 DS4_PORT=8080 ./deploy_ds4.sh start
DS4_MODEL_PATH=/path/to/model.gguf ./deploy_ds4.sh restart
```
